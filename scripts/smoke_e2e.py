...
def _build_sample_csv(index: int) -> str:
    fd, path = tempfile.mkstemp(prefix="docmind_smoke_", suffix=".csv")
    os.close(fd)
    with open(path, "w", encoding="utf-8") as f:
        f.write("policy,owner,amount\n")
        f.write(f"员工报销流程{index},财务部,{5000 + index}\n")
        f.write(f"差旅审批规则{index},行政部,{3000 + index}\n")
    return path
...
async def _delete_uploaded_docs(client: httpx.AsyncClient, doc_ids: list[str]) -> None:
...
async def main() -> None:
    ...
    sample_paths = [_build_sample_csv(index) for index in range(1, 4)]
    doc_ids: list[str] = []
    try:
        async with httpx.AsyncClient(base_url=args.base_url, timeout=45.0) as client:
            ...
            try:
                ...
                for index, sample_path in enumerate(sample_paths, start=1):
                    with open(sample_path, "rb") as f:
                        upload = await client.post(
                            ...
                            files={"file": (f"smoke_{index}.csv", f, "text/csv")},
                            data={"department": "Platform", "access_level": "2"},
                        )
                    upload.raise_for_status()
                    doc_id = upload.json()["id"]
                    doc_ids.append(doc_id)
                    terminal_statuses.append(await _wait_document_ready(client, doc_id))
                    ...
                    json.dumps(
                        {
                            "ok": True,
                            "doc_ids": doc_ids,
                            "document_statuses": [item.get("status") for item in terminal_statuses],
                ...
                await _delete_uploaded_docs(client, doc_ids)
    finally:
