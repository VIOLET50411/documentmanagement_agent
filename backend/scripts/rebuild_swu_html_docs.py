...
ROOT = BACKEND_ROOT if (BACKEND_ROOT / "datasets").exists() else BACKEND_ROOT.parent
...
def _load_manifest_sources() -> dict[str, Path]:
    ...
    for item in items:
        ...
        if not saved_path.exists():
            fallback = raw_root / str(item.get("filename") or "")
            if fallback.exists():
                saved_path = fallback
...
def _fetch_target_documents(max_chunks: int) -> list[dict]:
    sql = """
...
def _matches_patterns(doc: dict, include_patterns: list[str]) -> bool:
    if not include_patterns:
        return True
    haystacks = [str(doc.get("title") or ""), str(doc.get("file_name") or "")]
...
def main() -> None:
    ...
    documents = [doc for doc in _fetch_target_documents(args.max_chunks) if _matches_patterns(doc, args.include_pattern)]
    if args.limit > 0:
