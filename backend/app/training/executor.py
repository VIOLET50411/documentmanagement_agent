...
def _resolve_script_command_template(command_template: str | None = None) -> tuple[str, str]:
    ...
    if not settings.llm_training_executor_builtin_runner_enabled:
        raise ValueError("未配置训练脚本命令，且内置训练执行器已被禁用")

    runner_path = _builtin_training_runner_path()
    if not runner_path.exists():
        raise ValueError(f"内置训练脚本不存在: {runner_path}")

    python_executable = str(sys.executable or "").strip()
    if not python_executable:
        raise ValueError("无法解析 Python 可执行文件路径")
        ...
        payload.update(
            {
                ...
                "ready": True,
                "note": "当前使用 mock 训练执行器，不会产出真实 LoRA/SFT 结果",
            }
        )
        return payload
...
class TrainingExecutor:
    ...
    def validate_result(self, result: dict[str, Any]) -> dict[str, Any]:
        ...
        if not artifact_dir:
            raise ValueError("训练结果缺少 artifact_dir")
        if not serving_base_url:
            raise ValueError("训练结果缺少 serving_base_url")
        if not serving_model_name:
            raise ValueError("训练结果缺少 serving_model_name")
        if mode == "plan_only" or not executed:
            raise ValueError("训练结果仅输出训练计划，未执行真实训练")
        if not adapter_dir:
            raise ValueError("训练结果缺少 adapter_dir")
        if not Path(adapter_dir).exists():
            raise ValueError(f"训练结果中的 adapter_dir 不存在: {adapter_dir}")
        ...
        (artifact_dir / "model_card.md").write_text(
            "\n".join(
                [
                    ...
                    "",
                    "该模型卡由 DocMind mock 训练执行器生成，不代表真实 LoRA/SFT 训练产物。",
                ]
            ),
            encoding="utf-8",
            newline="\n",
...
class ScriptTrainingExecutor(TrainingExecutor):
    async def execute(self, request: TrainingExecutionRequest) -> dict[str, Any]:
        ...
        if process.returncode != 0:
            message = stderr_text or stdout_text or f"训练脚本执行失败，退出码 {process.returncode}"
    ...
    def _load_result_payload(self, artifact_dir: Path, stdout_text: str) -> dict[str, Any]:
        ...
        for line in reversed(stdout_text.splitlines()):
            ...
            if isinstance(payload, dict):
                return payload
        raise ValueError("训练脚本未输出可解析的结果 JSON")
...
class RemoteTrainingExecutor(TrainingExecutor):
    async def execute(self, request: TrainingExecutionRequest) -> dict[str, Any]:
        ...
        if not base_url:
            raise ValueError("未配置远程训练 API 地址")
        ...
        async with httpx.AsyncClient(timeout=httpx.Timeout(settings.llm_training_executor_timeout_seconds, connect=10.0)) as client:
            ...
            if not job_id and not status_url:
                raise ValueError("远程训练接口未返回 job_id 或 status_url")
        ...
        *,
        ...
        for _ in range(attempts):
            ...
            if status in {"failed", "error", "cancelled", "canceled", "killed"}:
                error_message = str((payload or {}).get("error") or (payload or {}).get("message") or "远程训练失败")
                raise RuntimeError(error_message)
        raise TimeoutError(f"远程训练在 {timeout_seconds}s 内未完成")
    ...
    def _extract_terminal_result(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(result_payload, dict):
            raise ValueError("远程训练返回了无法解析的结果结构")
        return result_payload
