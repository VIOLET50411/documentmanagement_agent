<template>
  <div class="documents-page workbench-page" :class="{ 'drawer-open': !!selectedDocForDetails }">
    ...
    <div class="stats-row">
      <div class="stat-card card">
        <span class="stat-value">{{ total }}</span><span class="stat-label">文档总数</span>
      </div>
      <div class="stat-card card">
        <span class="stat-value">{{ readyCount }}</span><span class="stat-label">已完成</span>
      </div>
      <div class="stat-card card">
    ...
    <div class="filter-bar card">
      <div class="filter-group">
        ...
        <select v-model="filters.department" class="input" @change="fetchDocuments" style="max-width: 140px;">
    ...
    <div class="documents-table card">
      <table class="data-table">
        ...
        <tbody>
          ...
          >
            ...
            <td class="doc-name">
              <span class="doc-icon">{{ fileIcon(doc.file_type) }}</span>{{ doc.title }}
            </td>
            <td>
              <span class="badge badge-primary">{{ doc.file_type?.split("/").pop() || "-" }}</span>
            </td>
            <td>{{ doc.department || "-" }}</td>
            <td>
              <span class="badge" :class="statusBadgeClass(doc.status)">{{ progressLabel(doc.status) }}</span>
            </td>
            <td>{{ doc.percentage ?? docStore.uploadProgress[doc.id]?.percentage ?? 0 }}%</td>
            <td class="text-muted">{{ formatDate(doc.created_at) }}</td>
            <td class="table-actions">
              <button class="btn btn-ghost btn-sm" @click="openDetailsDrawer(doc)">详情</button>
              <button class="btn btn-ghost btn-sm" :disabled="openingOriginalDocId === doc.id" @click="openOriginalDocument(doc)">
                {{ openingOriginalDocId === doc.id ? "打开中..." : "查看原文" }}
              </button>
              <button
                v-if="['failed', 'partial_failed', 'retrying'].includes(doc.status)"
      ...
      <div class="pagination-bar" v-if="total > 0">
        ...
        <div class="pagination-controls">
          ...
          <button class="btn btn-secondary btn-sm" :disabled="page * size >= total" @click="changePage(page + 1)">下一页</button>
        </div>
      </div>
    </div>

    <div class="side-drawer-overlay" v-if="selectedDocForDetails" @click="closeDetailsDrawer"></div>
    <aside class="side-drawer" :class="{ open: !!selectedDocForDetails }">
      <div v-if="selectedDocForDetails" class="drawer-content">
        <div class="drawer-header">
          <h3>文档详情</h3>
          <button class="btn btn-ghost btn-sm" @click="closeDetailsDrawer">关闭</button>
        </div>
        ...
        <div class="drawer-body">
          <div class="detail-block">
            <h4>基本信息</h4>
            <div class="info-grid">
              <span>文档名称</span><strong>{{ selectedDocForDetails.title }}</strong>
              <span>文档 ID</span><strong>{{ selectedDocForDetails.id }}</strong>
              <span>部门</span><strong>{{ selectedDocForDetails.department || "-" }}</strong>
              <span>上传时间</span><strong>{{ formatDate(selectedDocForDetails.created_at) }}</strong>
            </div>
          </div>

          <div class="detail-block">
            <h4>处理状态</h4>
            <div class="status-box" :class="statusBadgeClass(selectedDocStatus).replace('badge-', 'bg-')">
...
const processingCount = computed(
  () =>
    documents.value.filter((d) =>
      ["queued", "uploaded", "parsing", "chunking", "indexing", "retrying"].includes(d.status)
    ).length
)
...
const isAllSelected = computed(() => {
  ...
  return selectedDocIds.value.length === filteredAndSortedDocuments.value.length
})
...
const filteredAndSortedDocuments = computed(() => {
  ...
  if (filters.value.sortBy === "failed_first") {
    ...
    docs.sort((a, b) => {
      const aProc = ["queued", "uploaded", "parsing", "chunking", "indexing", "retrying"].includes(a.status) ? 1 : 0
      const bProc = ["queued", "uploaded", "parsing", "chunking", "indexing", "retrying"].includes(b.status) ? 1 : 0
      return bProc - aProc
    })
  }
...
async function fetchDocuments() {
  ...
  try {
    ...
    if (filters.value.status) {
      params.status = filters.value.status === "processing" ? "parsing" : filters.value.status
    }
...
async function bulkDelete() {
  if (!confirm(`确定要删除选中的 ${selectedDocIds.value.length} 个文档吗？`)) return
...
async function uploadFile(file: File) {
  ...
  for (let index = 0; index < totalParts; index += 1) {
    ...
    await documentsApi.uploadChunk((session as any).upload_id, partNumber, totalParts, blob)
  ...
  const completed = await documentsApi.completeUpload((session as any).upload_id)
...
async function openDetailsDrawer(doc: DocumentItem) {
  ...
  const [eventsRes, statusRes] = await Promise.all([documentsApi.getEvents(doc.id), documentsApi.getStatus(doc.id)])
...
function extractBatchFailures(events: DocEvent[], errorMessage: string) {
  const content = [errorMessage, ...events.map((e) => e.error), ...events.map((e) => e.detail)].filter(Boolean).join("；")
  const matches = content.match(/第\d+批失败[^；\n]*/g) || []
